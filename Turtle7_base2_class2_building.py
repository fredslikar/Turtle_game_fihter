...
def fire_allies_missile():
    start = random.choice(buildings)
    x2 = random.randint(-700, 700)
    y2 = random.randint(-270, 300)
    info = Missile(color="yellow", x=start.x, y=start.y, x2=x2, y2=y2)
    our_missiles.append(info)
    allies_missiles.append(info)
...
def check_allies_count():
    if len(allies_missiles) < ALLIES_COUNT:
        fire_allies_missile()
...
base = Building(x=BASE_X, y=BASE_Y, name="fighter")
...
while True:
    ...
    move_missiles(missiles=allies_missiles)
